...
import { deploySafeSingleOwner } from "./Safe";
import { readFileSync } from "fs";
import { MetaTransactionData } from "@safe-global/safe-core-sdk-types";

describe("ZeroKey", function () {
  // refer to https://github.com/PufferFinance/pufETH/blob/main/docs/PufferVaultV2.md
  ...
  async function deployFixture() {
    ...
    const safeEnableModuleTx = await safe.createEnableModuleTx(
      zeroKeyModuleAddress
    );
  ...
  describe("ZeroKeyModule functions", function () {
    it("Should set the hash and make a transaction", async function () {
      const { user, safe, safeAddress, zeroKeyModule, zeroKeyModuleAddress } =
      ...
      const proofJson = JSON.parse(
        readFileSync("zokrates/proof.json", "utf-8")
      );
      ...
      const transactions: MetaTransactionData[] = [
        {
          ...
          data: zeroKeyModule.interface.encodeFunctionData("setHash", [
            hash256,
          ]),
          value: "0",
        },
      ...
      await zeroKeyModule
        .connect(user)
        .executeTransactionWithProof(safeAddress, tx, proof, inputs);
    });
  });
});
